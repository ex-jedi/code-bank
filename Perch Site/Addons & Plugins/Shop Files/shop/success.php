...
<h1 class="main-heading"><a href="/"><?php perch_content('Sitewide Main Heading'); ?></a></h1>
		...
		<main class="main-content success-page">
			<h2 class="success-page-subtitle">Order Status</h2>
				<?php
				...
				if (perch_shop_order_successful()) {
					// Redisplay the details of the order
						perch_shop_order_items(perch_shop_successful_order_id());
						// Yes! Show a success message
						perch_content('Payment Success Message');
				}else{
				    // Payment problem! Show instructions to the customer
				    perch_content('Payment Not Successful Message');
				}
				?>

				<?php if(perch_member_logged_in()){ ?>
			 <a class="cart-logout-link sucess-log-out-link" href="/shop/logout.php">Log out</a>
				<?php } ?>
		</main> <!-- main content -->
